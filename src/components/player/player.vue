...
export default {
  ...
  methods: {
    ...
    middleTouchMove(e) {
      ...
      if (Math.abs(deltaY) > Math.abs(deltaX)) { // 纵向偏移大于横向偏移
        return
      }
    ...
    _getPosAndScale() {
      ...
      const paddingLeft = 40 // 目标中心坐标左边距离
      const paddingBottom = 30 // 相对底部距离
      const paddingTop = 80 // cd距顶部距离
      const width = window.innerWidth * 0.8 // cd宽度容器宽度
      const scale = targetWidth / width // 初始缩放比例
      const x = -(window.innerWidth / 2 - paddingLeft) // 目标横坐标
      const y = window.innerHeight - paddingTop - width / 2 - paddingBottom // 纵坐标
      return {
        x,
        y,
        scale
